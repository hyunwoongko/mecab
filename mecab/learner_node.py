...
def node_cmp_eq(node1: LearnerNode, node2: LearnerNode, size: int,
                unk_size: int) -> bool:
    if node1.length == node2.length and node1.surface[:node1.
                                                      length] == node2.surface[:node1
                                                                               .
                                                                               length]:
        p1 = node1.features
        p2 = node2.features
        ...
        if len(p1) - len(r1) == len(p2) - len(
                r2) and p1[:len(p1) - len(r1)] == p2[:len(p1) - len(r1)]:
            return True
...
def is_empty(path: LearnerPath) -> bool:
    return (
        ...
        or
        (not path.lnode.lpath and path.lnode.stat != NodeType.MECAB_BOS_NODE))
...
def calc_alpha(n: LearnerNode):
    ...
    path = n.lpath
    while path:
        n.alpha = logsumexp(n.alpha, path.cost + path.lnode.alpha,
                            path == n.lpath)
...
def calc_beta(n: LearnerNode):
    ...
    path = n.rpath
    while path:
        n.beta = logsumexp(n.beta, path.cost + path.rnode.beta,
                           path == n.rpath)
